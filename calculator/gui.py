...
from calculator.storage import load_data, save_data
...
class CalculatorApp:
    ...
    def load_saved_data(self):
        (self.history, self.memory, self.last_result) = load_data()

    def save(self):
        try:
            save_data(self.history, self.memory, self.last_result)

        except OSError:
            messagebox.showerror("Error", "Calculator data could not be saved.")
    ...
    def create_menu(self):
        ...
        file_menu.add_command(label="Clear", command=self.clear_display)
        ...
        memory_menu = tk.Menu(menu_bar, tearoff=False)

        memory_menu.add_command(
            label="Save Last Result", command=self.save_last_result_to_memory
        )

        memory_menu.add_command(label="View Memory", command=self.show_memory)
    ...
    def create_widgets(self):
        ...
        previous_label.pack(fill="x", pady=(0, 2))
        ...
        ttk.Button(tool_frame, text="ANS", command=self.use_last_result).pack(
            side="left", expand=True, fill="x", padx=(0, 4)
        )

        ttk.Button(tool_frame, text="History", command=self.show_history).pack(
            side="left", expand=True, fill="x", padx=4
        )

        ttk.Button(tool_frame, text="Memory", command=self.show_memory).pack(
    ...
    def calculate(self):
        ...
        try:
            ...
            messagebox.showerror("Error", "That number is too large.")
    ...
    def use_last_result(self):
        if self.last_result is None:
            messagebox.showinfo("Last Result", "No previous result is available.")
    ...
    def show_history(self):
        ...
        ttk.Label(
            frame, text="Calculation History", font=("Segoe UI", 16, "bold")
        ).pack(pady=(0, 10))

        history_box = tk.Text(frame, font=("Consolas", 11), wrap="word")

        history_box.pack(fill="both", expand=True)

        if not self.history:
            history_box.insert(tk.END, "No calculations in history.")
        ...
        ttk.Button(
            frame, text="Clear History", command=lambda: self.clear_history(window)
        ).pack(pady=(10, 0))
    ...
    def save_last_result_to_memory(self):
        if self.last_result is None:
            messagebox.showinfo("Memory", "There is no result to save.")
        ...
        ttk.Label(frame, text="Memory name:").pack(anchor="w")
        ...
        def save_memory_value():
            ...
            if name == "":
                messagebox.showerror("Error", "Memory name cannot be empty.")
        ...
        ttk.Button(frame, text="Save", command=save_memory_value).pack(pady=10)
    ...
    def show_memory(self):
        ...
        ttk.Label(frame, text="Saved Values", font=("Segoe UI", 16, "bold")).pack(
            pady=(0, 10)
        )

        if not self.memory:
            ttk.Label(frame, text="No saved values.").pack(pady=20)
        ...
        for name, value in self.memory.items():
            ...
            ttk.Label(row, text=(f"{name} = {format_number(value)}")).pack(
            ...
            ttk.Button(
                row,
                text="Use",
                command=lambda v=value, w=window: self.use_memory_value(v, w),
            ).pack(side="left", padx=4)

            ttk.Button(
                row,
                text="Delete",
                command=lambda n=name, w=window: self.delete_memory_value(n, w),
            ).pack(side="left")

    def use_memory_value(self, value, window):
        self.button_click(str(format_number(value)))

        window.destroy()

    def delete_memory_value(self, name, window):
    ...
    def create_keyboard_shortcuts(self):
        ...
        self.root.bind("<Control-h>", lambda event: self.show_history())

        self.root.bind("<Control-m>", lambda event: self.show_memory())

        self.root.bind("<Control-s>", lambda event: self.save_last_result_to_memory())

        self.root.bind("<Control-l>", lambda event: self.clear_display())
